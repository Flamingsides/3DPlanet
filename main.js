...
for (let i = 0; i <= numPoints; i++) {
  ...
  const x = radius * Math.cos(angle) * 1.5 - 50; // X coordinate of the circle (left right)
  const z = radius * Math.sin(angle) * 4; // Z coordinate of the circle (up down)
  const y = -i * 0; 
...
// Function to calculate t based on scroll position
function getScrollT() {
